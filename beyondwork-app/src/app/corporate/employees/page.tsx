'use client';

import { useAuthStore } from '@/lib/stores/auth-store';
import { useRouter } from 'next/navigation';
import { useEffect } from 'react';
import Link from 'next/link';

export default function CorporateEmployeesPage() {
  const { firebaseUser, userProfile, loading: authLoading } = useAuthStore();
  const router = useRouter();

  useEffect(() => {
    if (!authLoading && !firebaseUser) {
      router.push('/login');
    }
  }, [firebaseUser, authLoading, router]);

  if (authLoading || !firebaseUser) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p>Loading...</p>
      </div>
    );
  }

  return (
    <main className="min-h-screen bg-background p-8">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <Link
            href="/corporate"
            className="text-primary-600 hover:underline flex items-center gap-1"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            Back to Corporate Dashboard
          </Link>
        </div>

        <div className="bg-white rounded-lg shadow p-8">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h1 className="text-3xl font-bold">Employee Management</h1>
              <p className="text-gray-600 mt-1">{userProfile?.company}</p>
            </div>
            <button className="px-4 py-2 bg-primary-600 text-white rounded hover:bg-primary-700">
              Invite Employees
            </button>
          </div>

          {/* Invite Section */}
          <div className="bg-blue-50 border border-blue-200 rounded p-6 mb-8">
            <h3 className="font-semibold mb-2">Invite Employees via Email Domain</h3>
            <p className="text-sm text-gray-700 mb-4">
              Share the signup link with your team. Employees with {userProfile?.company} email domain will automatically be linked to your company.
            </p>
            <div className="bg-white p-3 rounded border flex items-center justify-between">
              <code className="text-sm">https://beyondwork.app/signup?company={userProfile?.company}</code>
              <button className="ml-2 px-3 py-1 bg-primary-600 text-white text-sm rounded hover:bg-primary-700">
                Copy Link
              </button>
            </div>
          </div>

          {/* Employee List */}
          <div>
            <h2 className="text-xl font-semibold mb-4">Company Employees</h2>
            <div className="text-center py-12 text-gray-500">
              <svg className="w-16 h-16 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
              </svg>
              <p>No employees registered yet</p>
              <p className="text-sm mt-2">Invite team members to get started</p>
            </div>
          </div>
        </div>
      </div>
    </main>
  );
}
